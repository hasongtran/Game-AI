...
def think(board, state):
    ...
    # select highest win rate node or winning node
    node = root_node
    curr_state = state
    action_list = []
    node_list = []
    ...
    #heuristics: first check if we can win a box, if so take it
    #if there are multiple win locations, choose the one with best rate
    for i in node.child_nodes:
        #get new state for child node with applied move and get dict of winned boxes
        temp = node.child_nodes[i]
        curr_state = board.next_state(state, temp.parent_action)
        box_win_dict = board.owned_boxes(curr_state)
        y, x, a, b = temp.parent_action
        
        #check if we win the box with the applied move
        if box_win_dict[(y,x)] == identity_of_bot:
            rate = (temp.wins / temp.visits)
            if action_list and rate_list[0]< rate:
                rate_list.insert(0, rate)
                action_list.insert(0, temp.parent_action)
            else:
                action_list.append(temp.parent_action)
                rate_list.append(rate)
    #if there is a win for a box, return best rate action
    if action_list:
        return action_list[0]
    
    #reset and get best win rate node
    rate_list = []
    curr_state = state
    for i in node.child_nodes:
        temp = node.child_nodes[i]
